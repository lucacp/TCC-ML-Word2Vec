...
#    print(numlin)
    ...
    arq.close()
    arq   = None
    arq1  = codecs.open(nmvc+"-peso1.txt",'w','ascii')
    arq2  = codecs.open(nmvc+"-peso2.txt",'w','ascii')
    num   = 0
    X1    = []
    X2    = []
    y     = []
    ...
    for line in sp:
        ...
        for one in line.split():
            ...
            if firs == 1:
                #print(cnt[one])
                mat1 = A1 * (model[one] * prob)
                mat2 = A2 * (model[one] * prob)
            else:
                try:
                    if firs < linh:
                        mat1 = mat1 + ( A1 * model[one]) * prob
                        mat2 = mat2 + ( A2 * model[one]) * prob
                    else:
                        y.append(int(one))
                except:
...
#    print(numlin)
    ...
    arq1  = codecs.open(nmvc+"-peso1-st.txt",'w','ascii')
    arq2  = codecs.open(nmvc+"-peso2-st.txt",'w','ascii')
    num   = 0
    X1    = []
    X2    = []
    y     = []
...
for i in arqs:
    print(i)
    somaProbp(i,dados)
    #somaProbpSw(i,dados,sw) usando peso os vetores sao zerados
