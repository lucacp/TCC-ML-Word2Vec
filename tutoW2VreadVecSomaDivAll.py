...
def somaDiv(txt,nmdt): #txt: nome_do_corpus ; nmdt: arquivo_das_respostas
    nmvc  = txt
    ...
    arq.close()
    arq   = None
    arq   = codecs.open(nmvc+"-div.txt",'w','ascii')
    num   = 0
    X     = []
    y     = []
    for line in sp:
...
def somaDivSw(txt,nmdt,sw): #txt: nome_do_corpus ; nmdt: arquivo_das_respostas ; sw: nome_arquivo_stopwords
    nmvc  = txt
    model = Word2Vec.load(nmvc)
    model.init_sims(replace=True)
    #pprint(model.wv.vocab)
    ...
    arq   = codecs.open(nmvc+"-div-st.txt",'w','ascii')
    num   = 0
    X     = []
    y     = []
    for line in sp:
