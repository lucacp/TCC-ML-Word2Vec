...
def somaRed(txt,nmdt): #txt: nome_do_corpus ; nmdt: arquivo_das_respostas
    nmvc  = txt  #str(sys.argv[1]) #nome_corpus
...
def somaRedSw(txt,nmdt,sw): #txt: nome_do_corpus ; nmdt: arquivo_das_respostas ; sw: nome_arquivo_stopwords
    nmvc  = txt  #str(sys.argv[1]) #nome_corpus
    model = Word2Vec.load(nmvc)
    model.init_sims(replace=True)
    #pprint(model.wv.vocab)
    ...
    arq1 = open('SomaRed-pca_n_' + nmvc + "-st.txt" , 'w' )
    arq2 = open('SomaRed-pca_w_' + nmvc + "-st.txt" , 'w' )
    arq3 = open('SomaRed-gaussi_'+ nmvc + "-st.txt" , 'w' )
    arq4 = open('SomaRed-sparse_'+ nmvc + "-st.txt" , 'w' )
...
ar    = None
ar    = open(str(sys.argv[1]))  #Corpus_file
dados =      str(sys.argv[2])   #Arquivo_respostas
tex   = ar.read()
ar.close()
ar    = None
arqs  = tex.split('\n')
sw    =      str(sys.argv[3])   #Arquivo_stopwords
for i in arqs:
    print(i)
    somaRed(i,dados)
    somaRedSw(i,dados,sw)
