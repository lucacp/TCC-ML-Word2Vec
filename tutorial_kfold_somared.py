...
def kfold_resultados_somared(data_file,nome):
    ...
    #gera_pickle('Vec-SomaRed-pca_n_' +data_file, targets,'SR-PN' + nome+".pkl")
    #gera_pickle('Vec-SomaRed-pca_w_' +data_file, targets,'SR-PW' + nome+".pkl")
    #gera_pickle('Vec-SomaRed-sparse_'+data_file, targets,'SR-S-' + nome+".pkl")


    X, y = carrega_pickle(nome + ".pkl")
    #X2, y = carrega_pickle('SR-PN' + nome + ".pkl")
    #X3, y = carrega_pickle('SR-PW' + nome + ".pkl")
    #X4, y = carrega_pickle('SR-S-' + nome + ".pkl")
    #X1 = X1[:62]
    #X2 = X2[:62]
    #X3 = X3[:62]
    #X4 = X4[:62]
    ...
    arq = open('Result-'+ data_file , 'w' )
    
    pca_regular = IncrementalPCA(n_components=1, whiten=False)
    pca_whiten = IncrementalPCA(n_components=1, whiten=True)
    #pca_regular = PCA(n_components=1, whiten=False)
    #pca_whiten = PCA(n_components=1, whiten=True)
...
#        grid_search2.fit(X2, y)#aqui o programa fica parado pois os numeros dentro do 'respostas_pca_whiten' sao todos NaN
...
for i in arqs:
    ...
    #print(i+"-st.txt")
    #l=i+"-st.txt"
    print('Vec-SomaRed-gaussi_'+i+".txt")
    l='Vec-SomaRed-gaussi_'+i+".txt"
    kfold_resultados_somared(data_file=l,nome='SR-G'+i)
    l=None
    print('Vec-SomaRed-gaussi_'+i+"-st.txt")
    l='Vec-SomaRed-gaussi_'+i+"-st.txt"
    kfold_resultados_somared(data_file=l,nome='SR-G'+i)
    l=None
    print('Vec-SomaRed-pca_n_'+i+".txt")
    l='Vec-SomaRed-pca_n_'+i+".txt"
    kfold_resultados_somared(data_file=l,nome='SR-PN'+i)
    l=None
    print('Vec-SomaRed-pca_n_'+i+"-st.txt")
    l='Vec-SomaRed-pca_n_'+i+"-st.txt"
    kfold_resultados_somared(data_file=l,nome='SR-PN'+i)
    l=None
    print('Vec-SomaRed-pca_w_'+i+".txt")
    l='Vec-SomaRed-pca_w_'+i+".txt"
    kfold_resultados_somared(data_file=l,nome='SR-PW'+i)
    l=None
    print('Vec-SomaRed-pca_w_'+i+"-st.txt")
    l='Vec-SomaRed-pca_w_'+i+"-st.txt"
    kfold_resultados_somared(data_file=l,nome='SR-PW'+i)
    l=None
    print('Vec-SomaRed-sparse_'+i+".txt")
    l='Vec-SomaRed-sparse_'+i+".txt"
    kfold_resultados_somared(data_file=l,nome='SR-S'+i)
    l=None
    print('Vec-SomaRed-sparse_'+i+"-st.txt")
    l='Vec-SomaRed-sparse_'+i+"-st.txt"
    kfold_resultados_somared(data_file=l,nome='SR-S'+i)
    l=None
