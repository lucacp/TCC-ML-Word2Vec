...
def concatenaSw(nmvc,nmdt,sw):
    ...
    for line in sp:
        ...
        XX.append(Y)
        #X.append(mat)
    arq1 = open('Concat_-pca_n_'+nmvc+"-st.txt",'w')
    arq2 = open('Concat_-pca_w_'+nmvc+"-st.txt",'w')
    arq3 = open('Concat_-gaussi_'+nmvc+"-st.txt",'w')
    arq4 = open('Concat_-sparse_'+nmvc+"-st.txt",'w')
